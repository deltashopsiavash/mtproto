...
def reload_shared_proxy(users: Iterable):
    ...
    # Important: MTProxy inside normal Docker bridge often advertises/uses the
    # container private IP and Telegram clients fail to connect. MTPulse works
    # because it runs on the host network. We do the same here: host network,
    # no Docker port publishing. The proxy binds directly to PROXY_PORT on the VPS.
    return c.containers.run(
        ...
        environment={"PROXY_PORT": str(PROXY_PORT), "PUBLIC_HOST": public_host()},
        volumes={"mtproto_proxy_config": {"bind": "/data", "mode": "rw"}},
        labels={"managed-by": "delta-proxy-panel", "proxy-mode": "shared-single-port-hostnet"},
    )
