...
install_deps(){ apt-get update -y; apt-get install -y git curl wget build-essential zlib1g-dev libssl-dev python3 python3-venv python3-pip iptables rsync; }
install_mtproxy(){
...
write_config(){
  mkdir -p "$ETC_DIR"
  local panel_port username password proxy_ports hash public_ip
