...
This build intentionally does not allocate shared-port firewall traffic to users. With the official `mtproto-proxy` binary and all users sharing one public port, Linux firewall counters can only see total traffic on that port and cannot identify which Secret used it. If `/stats` does not expose Secret-specific rows, per-user usage stays at 0 instead of creating fake usage for new/unused users.

Open `/traffic-debug` in the panel to inspect the raw local MTProxy stats output.
