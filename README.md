...
## Proxy engine fix v3
This version uses the official Telegram MTProxy binary and creates Telegram links with the raw `-S` secret, matching the working MTPulse command format.


## v4 fix
MTProxy now runs with Docker host networking, exactly like host-installed MTPulse behavior. This avoids Docker bridge/NAT issues that can make Telegram clients unable to connect to generated proxies.
