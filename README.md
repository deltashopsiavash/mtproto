...
- Public MTProto port stays shared for all users.
- Each user now gets a TLS-mode Secret with a hidden SNI route and a private local backend.
- HAProxy routes the one public port to the correct backend, so usage/quota counters are separated per user instead of sharing the same port counter.
- After this update, copy the new proxy links from the panel because old classic 32-hex secrets are migrated to TLS-mode secrets.
