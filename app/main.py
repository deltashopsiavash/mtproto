...
def sync_traffic() -> None:
    ...
    # Shared-port mode cannot identify Telegram MTProto users by secret from Linux iptables.
    # Old versions divided every new byte between all enabled users, which made offline users lose quota.
    # v15 changes that: traffic is applied ONLY to users recently touched from their sub-link/panel.
    # If no recent owner is known, bytes are kept as shared_unassigned_bytes and no user quota is reduced.
    with db() as conn:
        prev_total = int(setting("shared_total_bytes_last", "0") or 0)
        delta_bytes = max(0, total_bytes - prev_total)
        rows = conn.execute("SELECT id,quota_gb,used_gb,enabled,last_seen FROM users").fetchall()
        enabled_rows = [r for r in rows if int(r["enabled"])]
        owners = [r for r in enabled_rows if _recent_dt(r["last_seen"], 900)]

        # If there is exactly one enabled user, attribute to it. If more than one user exists and none
        # is recently active, do NOT charge anyone; this prevents offline proxies from losing volume.
        if not owners and len(enabled_rows) == 1:
            owners = enabled_rows

        if delta_bytes > 0 and owners:
            delta_gb_each = (delta_bytes / (1024**3)) / max(1, len(owners))
            for r in owners:
                conn.execute("UPDATE users SET used_gb=COALESCE(used_gb,0)+? WHERE id=?", (delta_gb_each, r["id"]))
        elif delta_bytes > 0:
        ...
        set_setting(conn, "shared_active_owner_count", str(len(owners)))
        conn.commit()
