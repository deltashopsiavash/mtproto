...
def sync_traffic() -> None:
    ...
    # Shared-port mode has one socket/port for all users, so Linux counters cannot identify the secret.
    # We still keep real shared-port byte counters and distribute only the *new delta* across enabled users
    # as an approximate per-user usage, so values never show as unlimited/empty.
    with db() as conn:
        prev_total = int(setting("shared_total_bytes_last", "0") or 0)
        delta_bytes = max(0, total_bytes - prev_total)
        rows = conn.execute("SELECT id,quota_gb,used_gb,enabled FROM users").fetchall()
        active_rows = [r for r in rows if int(r["enabled"])]
        active_count = max(1, len(active_rows))
        if delta_bytes > 0 and active_rows:
            delta_gb_each = (delta_bytes / (1024**3)) / active_count
            for r in active_rows:
                conn.execute("UPDATE users SET used_gb=COALESCE(used_gb,0)+?, last_seen=COALESCE(?, last_seen) WHERE id=?", (delta_gb_each, last_seen, r["id"]))
        elif last_seen:
            for r in active_rows:
        ...
        set_setting(conn, "shared_approx_gb_each", str((total_bytes / (1024**3)) / active_count))
        conn.commit()
...
def public_sub(secret: str):
    ...
    remain = None if quota <= 0 else max(0, quota - used)
    days = days_left(r["expire_at"])
    day_pct = 0
    if r["expire_at"] and days is not None:
        try:
            created = datetime.fromisoformat(r["created_at"].replace("Z", "+00:00"))
            end = datetime.fromisoformat(r["expire_at"].replace("Z", "+00:00"))
            total_days = max(1, (end - created).days)
            day_pct = max(0, min(100, int(round((days / total_days) * 100))))
        except Exception:
            day_pct = 0
    q_pct = 100 - pct_used(used, quota) if quota > 0 else 100
    status = "فعال" if int(r["enabled"] or 0) and is_not_expired(r["expire_at"]) else "غیرفعال"
    title = (f"{quota:g} گیگ" if quota > 0 else "حجم نامحدود")
    duration = "نامحدود" if days is None else f"{days} روز"
    remain_text = "∞" if remain is None else f"{remain:.2f}".rstrip("0").rstrip(".")
    link = proxy_link(r["secret"])
    safe_name = html_escape(r["name"])
    initial = html_escape(str(r["name"])[:1].upper())
    html = f"""
<!doctype html><html lang="fa" dir="rtl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{safe_name}</title>
<style>
*{{box-sizing:border-box}}body{{margin:0;min-height:100vh;font-family:Tahoma,Arial,sans-serif;background:linear-gradient(145deg,#f4b66c,#ed8834);color:#223536;display:flex;align-items:center;justify-content:center;padding:22px}}.wrap{{width:min(760px,100%)}}.top{{display:flex;align-items:center;gap:18px;margin-bottom:22px;direction:ltr}}.avatar{{width:105px;height:105px;border-radius:50%;background:#f4eadf;border:2px solid rgba(255,255,255,.75);box-shadow:0 12px 30px rgba(0,0,0,.12);display:grid;place-items:center;font-size:44px;font-weight:900;color:#e18836}}.name{{font-size:38px;font-weight:900}}.port{{font-size:26px;opacity:.9}}.brand{{margin-left:auto;font-size:40px;font-weight:900}}.card{{background:rgba(235,126,37,.72);border:1px solid rgba(65,40,20,.25);border-radius:34px;padding:42px 28px;box-shadow:0 24px 70px rgba(91,45,9,.23), inset 0 1px 0 rgba(255,255,255,.2)}}.inner{{background:#f6e9e2;border-radius:32px;padding:28px 24px;text-align:center;box-shadow:inset 0 1px 0 rgba(255,255,255,.9)}}h1{{font-size:34px;margin:0 0 22px}}.grid{{display:grid;grid-template-columns:1fr 1fr;gap:20px}}.metric b{{font-size:38px;color:#0d1720}}.metric span{{font-size:25px}}.battery{{margin:15px auto 0;width:118px;height:48px;border-radius:13px;border:7px solid #99a2aa;background:#333943;position:relative;box-shadow:inset 0 0 0 3px rgba(255,255,255,.22)}}.battery:before{{content:"";position:absolute;right:-18px;top:13px;width:12px;height:18px;border-radius:4px;background:#8b969e}}.fill{{position:absolute;inset:5px;border-radius:6px;background:#55cf75;display:grid;place-items:center;color:#873951;font-weight:900;min-width:30px}}.dark .fill{{background:#23272f;color:#fff}}.actions{{display:flex;gap:10px;margin-top:18px;direction:ltr}}button,a.btn{{border:0;border-radius:16px;padding:13px 16px;background:#263838;color:#fff;text-decoration:none;font-weight:800;cursor:pointer;flex:1;text-align:center}}.sub{{margin-top:12px;text-align:center;color:#334;opacity:.82}}@media(max-width:560px){{.top{{gap:12px}}.avatar{{width:76px;height:76px;font-size:32px}}.name{{font-size:28px}}.port{{font-size:20px}}.brand{{font-size:30px}}.grid{{grid-template-columns:1fr}}h1{{font-size:27px}}}}
</style></head><body><main class="wrap"><div class="top"><div class="avatar">{initial}</div><div><div class="name">{safe_name}</div><div class="port">{base_proxy_port()}</div></div><div class="brand">وی نت</div></div><section class="card"><div class="inner"><h1>{html_escape(duration)} {html_escape(title)}</h1><div class="grid"><div class="metric"><span><b>{'∞' if days is None else days}</b> روز باقی مانده</span><div class="battery"><div class="fill" style="width:{day_pct}%">{day_pct}%</div></div></div><div class="metric"><span><b>{remain_text}</b> گیگ باقی مانده</span><div class="battery dark"><div class="fill" style="width:{q_pct}%">{q_pct}%</div></div></div></div><div class="sub">وضعیت: {html_escape(status)} · مصرف شده: {fmt_gb(used, False)} از {fmt_gb(quota)}</div><div class="actions"><button onclick="navigator.clipboard.writeText('{link}')">کپی لینک پروکسی</button><a class="btn" href="{link}">اتصال</a></div></div></section></main></body></html>
"""
...
def export_backup(req: Request):
    require(req)
    data = backup_payload()
    return Response(json.dumps(data, ensure_ascii=False, indent=2), media_type="application/json", headers={"Content-Disposition": "attachment; filename=delta-mtproto-backup-v10.json"})
