...
def sync_traffic() -> None:
    ...
    # IMPORTANT: In shared-port mode stock MTProxy + iptables can only see traffic for the
    # shared port, not which secret/user generated it. Any attempt to split bytes between
    # users is wrong and causes the exact bug where offline users lose quota or one user's
    # traffic is charged to another. v16 therefore never auto-adds shared-port traffic to
    # individual users. It only stores total shared traffic and enforces users that are
    # already expired/disabled/over-quota by removing their secret from the MTProxy config.
    needs_render = False
    with db() as conn:
        ...
        set_setting(conn, "shared_active_owner_count", "0")
        set_setting(conn, "traffic_accounting_mode", "shared_strict_no_per_user")
        conn.commit()
    if needs_render:
        render_proxy_service()
