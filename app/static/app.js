...
function card(title,value,sub,bar,extra=''){ return `<div class="stat-card"><span>${title}</span><strong>${value}</strong><small>${sub}</small>${bar!==undefined?`<div class="progress"><i style="width:${Math.min(100,bar)}%"></i></div>`:''}${extra}</div>`; }
async function loadOverview(){ const o=await fetch('/api/overview').then(r=>r.json()); applyTheme(o.theme); $('#cards').innerHTML = [
  ...
  card('Real Traffic',`↓ ${fmtBytes(o.rx)}`,`↑ ${fmtBytes(o.tx)} کل سرور`),
  card('Online',`${o.online_total}`,`اتصال لحظه‌ای روی پورت مشترک`),
  card('Proxy',escapeHtml(o.proxy_status),`${escapeHtml(o.public_host)} / shared ${o.proxy_port}`),
  card('Users',`${o.users_enabled}/${o.users_total}`,'فعال / کل'),
  card('Quota',fmtGB(o.used_total),`از ${fmtGB(o.quota_total)} مصرف ثبت‌شده کاربران`, o.quota_total>0 ? (o.used_total/o.quota_total)*100 : 0),
...
function renderUsers(){ const q=($('#searchUser')?.value||'').toLowerCase(); const users=allUsers.filter(u=>(u.name+u.secret+(u.note||'')+(u.port||'')).toLowerCase().includes(q)); $('#userList').innerHTML = users.map(u=>userCard(u)).join('') || '<p class="empty">هنوز کاربری ساخته نشده.</p>'; }
function userCard(u){ const pct=u.usage_percent||0; const online=u.online||0; return `<article class="user-card ${u.enabled?'':'off'} ${u.expired?'expired':''}">
  <div class="user-top"><div><h3>${escapeHtml(u.name)}</h3><p><b class="dot ${online?'on':''}"></b>${online} آنلاین · ${u.enabled?'فعال':'غیرفعال'} ${u.expired?'· منقضی شده':''} ${u.disabled_reason==='quota'?'· پایان حجم':''}</p></div><button class="mini" onclick="toggleUser(${u.id})">${u.enabled?'غیرفعال':'فعال'}</button></div>
  <input class="link" readonly value="${u.link}" onclick="this.select()" />
  <div class="usage"><div><b>${fmtGB(u.used_gb)}</b><small>مصرف ثبت‌شده از ${fmtGB(u.quota_gb)} · Shared Port</small></div><span>${pct}%</span></div><div class="progress"><i style="width:${pct}%"></i></div>
  <details><summary>ویرایش حجم، تاریخ و وضعیت</summary><form class="edit-form" onsubmit="updateUser(event,${u.id})"><label>نام</label><input name="name" value="${escapeHtml(u.name)}" required /><div class="row"><div><label>افزایش/تنظیم اعتبار از امروز / روز</label><input name="expire_days" type="number" min="0" step="1" placeholder="${calcDaysLeft(u)}" /></div><div><label>حجم GB</label><input name="quota_gb" type="number" min="0" step="0.1" value="${u.quota_gb||0}" /></div></div><label class="check"><input name="keep_expire" type="checkbox" value="1" checked /> اگر روز جدید خالی بود، تاریخ فعلی حفظ شود</label><label>مصرف GB</label><input name="used_gb" type="number" min="0" step="0.1" value="${Number(u.used_gb||0).toFixed(3)}" /><label>وضعیت</label><select name="enabled"><option value="1" ${u.enabled?'selected':''}>فعال</option><option value="0" ${!u.enabled?'selected':''}>غیرفعال</option></select><label>یادداشت</label><textarea name="note" rows="2">${escapeHtml(u.note||'')}</textarea><button class="primary">ذخیره تغییرات</button></form></details>
  <div class="meta"><span>انقضا: ${isoDate(u.expire_at)}</span><span>پورت مشترک: ${u.shared_port||u.port}</span><span>آخرین اتصال: ${isoDate(u.last_seen)}</span><span>Secret: <code>${u.secret}</code></span></div>
...
$('#userForm')?.addEventListener('submit',async e=>{ e.preventDefault(); const r=await fetch('/api/users',{method:'POST',body:new FormData(e.target)}); if(!r.ok){toast('خطا در ساخت کاربر'); return;} e.target.reset(); toast('کاربر ساخته شد'); await loadUsers(); await loadOverview(); });
async function updateUser(e,id){ e.preventDefault(); const r=await fetch(`/api/users/${id}/update`,{method:'POST',body:new FormData(e.target)}); toast(r.ok?'کاربر آپدیت شد':'خطا در آپدیت'); await loadUsers(); await loadOverview(); }
