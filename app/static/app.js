...
function gbToBest(gb){ gb=Number(gb||0); if(gb<=0) return 'نامحدود'; const mb=gb*1024; if(gb<1) return `${fmtNum(mb,0)} MB`; return `${fmtNum(gb,2)} GB`; }
...
async function loadOverview(){ const o=await fetch('/api/overview').then(r=>r.json()); applyTheme(o.theme); $('#cards').innerHTML = [
  ...
  card('Quota',gbToBest(o.used_total),`از ${gbToBest(o.quota_total)} مصرف ثبت‌شده`, o.quota_total>0 ? (o.used_total/o.quota_total)*100 : 0),
...
function userCard(u){ const pct=Number(u.usage_percent||0); const online=Number(u.online||0); const quota=Number(u.quota_gb||0); const used=Number(u.used_gb||0); return `<article class="user-card ${u.enabled?'':'off'} ${u.expired?'expired':''}">
  <div class="user-main">
    ...
    <div class="user-quota"><strong>${gbToBest(used)}</strong><small>از ${gbToBest(quota)}</small></div>
  ...
  <div class="link-row"><input class="link" readonly value="${u.link}" onclick="this.select()" /><button onclick="copyText('${u.link}')">کپی</button></div>
