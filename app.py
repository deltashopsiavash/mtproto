...
def mtp_engine_secret(secret):
    """Return the 32-hex base secret expected by mtprotoproxy."""
    s=(secret or '').strip().lower()
    if len(s) in (34,66) and s[:2] in ('dd','ee'):
        return s[2:34]
    return s[:32]

def mtp_metric_name(username):
    return re.sub(r'[^A-Za-z0-9_]', '_', str(username or 'user')) or 'user'
...
def traffic_total(port=None):
    ...
    with db() as con:
        ...
        con.execute('create table if not exists user_traffic_state(username text primary key,last_raw integer not null default 0,total_bytes integer not null default 0,updated_at text)')
...
def mtproxy_stats_text():
    """Return local debug stats from official MTProxy if present.

    Official Telegram MTProxy stats are global only and cannot separate users on a
    shared port. The panel therefore uses mtprotoproxy journald stats for quota.
    """
    port=stats_port()
...
def _parse_size_to_bytes(amount, unit):
    try: v=float(str(amount).replace(',','.'))
    except Exception: return 0
    u=(unit or '').lower()
    if u.startswith('g'): return int(v*1000000000)
    if u.startswith('m'): return int(v*1000000)
    if u.startswith('k'): return int(v*1000)
    return int(v)

def mtprotoproxy_journal_text(lines=3000):
    return sh(f"journalctl -u mtproxy-shared.service --no-pager -n {int(lines)} -o cat 2>/dev/null || true").stdout or ''

def user_total_raw(username):
    """Read cumulative per-user bytes from mtprotoproxy journald stats."""
    name=str(username or '').strip()
    if not name:
        return 0
    text=mtprotoproxy_journal_text()
    if not text:
        return 0
    latest=0
    pat=re.compile(r'(?m)^%s:\s*\d+\s+connects\s*\([^)]*\),\s*([0-9]+(?:[\.,][0-9]+)?)\s*([KMGT]?B|bytes?)\b' % re.escape(name), re.I)
    for m in pat.finditer(text):
        latest=_parse_size_to_bytes(m.group(1), m.group(2))
    return max(0,latest)

def user_total(username):
    """Persistent monotonic per-user total based on mtprotoproxy stats."""
    user=str(username or '').strip(); raw=user_total_raw(user); now=datetime.utcnow().isoformat()
    if not user:
        return 0
    with db() as con:
        con.execute('create table if not exists user_traffic_state(username text primary key,last_raw integer not null default 0,total_bytes integer not null default 0,updated_at text)')
        row=con.execute('select * from user_traffic_state where username=?',(user,)).fetchone()
        if not row:
            con.execute('insert into user_traffic_state(username,last_raw,total_bytes,updated_at) values(?,?,?,?)',(user,raw,0,now))
            return 0
        last=int(row['last_raw'] or 0); total=int(row['total_bytes'] or 0)
        if raw>=last:
            total += raw-last
        con.execute('update user_traffic_state set last_raw=?,total_bytes=?,updated_at=? where username=?',(raw,total,now,user))
        return total
...
def reset_user_usage(uid):
    with db() as con:
        ...
        if r:
            con.execute('update users set used_reset_bytes=? where id=?',(user_total(r['username']),uid))

def used_bytes(r):
    return max(0, user_total(r['username'])-int(r['used_reset_bytes'] or 0))
...
def write_mtprotoproxy_config(rows):
    ...
    for r in rows:
        ...
        users[uname]=mtp_engine_secret(r['secret'])
        qb=quota_bytes(r)
        used=max(0, user_total(uname)-int(r['used_reset_bytes'] or 0))
        if qb>0:
            quotas[uname]=max(0, qb-used)
        try:
            exp=datetime.fromisoformat(r['expires_at']).date()
            expirations[uname]=exp.strftime('%d/%m/%Y')
        except Exception:
            pass
    metrics_port=int(cfg().get('metrics_port') or 9109)
    conf = (
        '# Auto-generated by MTProto Panel. Do not edit manually.\n'
        f'PORT = {int(shared_proxy_port())}\n'
        'LISTEN_ADDR_IPV4 = "0.0.0.0"\n'
        'LISTEN_ADDR_IPV6 = None\n'
        f'USERS = {repr(users)}\n'
        f'USER_DATA_QUOTA = {repr(quotas)}\n'
        f'USER_EXPIRATIONS = {repr(expirations)}\n'
        'MODES = {"classic": True, "secure": True, "tls": True}\n'
        'TLS_DOMAIN = "www.google.com"\n'
        'MASK = True\n'
        'USE_MIDDLE_PROXY = False\n'
        'STATS_PRINT_PERIOD = 5\n'
        f'METRICS_PORT = {metrics_port}\n'
        'METRICS_LISTEN_ADDR_IPV4 = "127.0.0.1"\n'
        'METRICS_LISTEN_ADDR_IPV6 = None\n'
        'METRICS_WHITELIST = ["127.0.0.1", "::1"]\n'
    )
    path=ETC_DIR/'mtprotoproxy_config.py'
...
def restart_shared_proxy():
    ...
    with db() as con: rows=con.execute('select * from users where enabled=1').fetchall()
    sh('systemctl stop mtproxy-shared.service >/dev/null 2>&1 || true')
    sh("pkill -f 'mtproto-proxy.*-H %s' >/dev/null 2>&1 || true" % port)
    sh("pkill -f 'mtprotoproxy.*mtprotoproxy_config.py' >/dev/null 2>&1 || true")
    if not rows:
        sh('systemctl disable mtproxy-shared.service >/dev/null 2>&1 || true')
        return
    conf_path=write_mtprotoproxy_config(rows)
    pybin=APP_DIR/'venv/bin/python'
    if not pybin.exists(): pybin=Path('/usr/bin/python3')
    mtpbin=APP_DIR/'venv/bin/mtprotoproxy'
    if mtpbin.exists():
        start_cmd=f"exec {q(mtpbin)} {q(conf_path)}"
    else:
        start_cmd=f"exec {q(pybin)} {q(APP_DIR/'venv/bin/mtprotoproxy.py')} {q(conf_path)}"
    unit=f"""[Unit]\nDescription=Shared MTProto Proxy Multi User Engine\nAfter=network.target\n\n[Service]\nType=simple\nWorkingDirectory={APP_DIR}\nExecStart=/bin/sh -lc {q(start_cmd)}\nRestart=always\nRestartSec=3\nLimitNOFILE=65535\nKillMode=control-group\nTimeoutStopSec=5\nEnvironment=PYTHONUNBUFFERED=1\n\n[Install]\nWantedBy=multi-user.target\n"""
...
def import_backup():
    ...
    with db() as con:
        ...
        for u in data.get('users',[]):
            ...
            con.execute('insert into users(username,port,secret,quota_gb,quota_bytes,expires_at,enabled,created_at,note,sub_token,used_reset_bytes) values(?,?,?,?,?,?,?,?,?,?,?)',(u['username'],shared_proxy_port(),u['secret'],qgb,qb,u.get('expires_at') or date.today().isoformat(),int(u.get('enabled',1)),u.get('created_at') or datetime.utcnow().isoformat(),u.get('note',''),u.get('sub_token') or secrets.token_urlsafe(16),int(u.get('used_reset_bytes') or user_total(u.get('username','')))))
