...
def _allocated_user_total(username):
    """Fallback usage counter for official MTProxy shared-port mode.

    Official Telegram MTProxy does not reliably expose per-Secret byte counters on
    every build. When /stats has no per-Secret rows, this keeps proxies working
    with the official engine and allocates only NEW shared-port traffic from the
    moment this version is installed/started. This prevents newly created users
    from inheriting old traffic from the shared port, and avoids the broken 0-byte
    display when /stats is unavailable.
    """
    user=str(username or '').strip(); now=datetime.utcnow().isoformat()
    if not user:
        return 0
    port=shared_proxy_port(); total=traffic_total(port)
    with db() as con:
        con.execute('create table if not exists user_traffic_alloc(username text primary key,total_bytes integer not null default 0,created_at text)')
        con.execute('create table if not exists alloc_state(port integer primary key,last_total integer not null default 0,updated_at text)')
        row=con.execute('select * from user_traffic_alloc where username=?',(user,)).fetchone()
        if not row:
            con.execute('insert into user_traffic_alloc(username,total_bytes,created_at) values(?,?,?)',(user,0,now))
        st=con.execute('select * from alloc_state where port=?',(port,)).fetchone()
        if not st:
            con.execute('insert into alloc_state(port,last_total,updated_at) values(?,?,?)',(port,total,now))
            row=con.execute('select * from user_traffic_alloc where username=?',(user,)).fetchone()
            return int(row['total_bytes'] or 0)
        last=int(st['last_total'] or 0); delta=max(0,int(total)-last)
        if delta>0:
            active=con.execute('select username from users where enabled=1 order by id').fetchall()
            names=[str(x['username']) for x in active if str(x['username']).strip()]
            if names:
                share=delta//len(names); rem=delta%len(names)
                for i,n in enumerate(names):
                    con.execute('insert or ignore into user_traffic_alloc(username,total_bytes,created_at) values(?,?,?)',(n,0,now))
                    con.execute('update user_traffic_alloc set total_bytes=total_bytes+? where username=?',(share+(1 if i<rem else 0),n))
            con.execute('update alloc_state set last_total=?,updated_at=? where port=?',(int(total),now,port))
        row=con.execute('select * from user_traffic_alloc where username=?',(user,)).fetchone()
        return int(row['total_bytes'] or 0) if row else 0

def reset_user_usage(uid):
    with db() as con:
        r=con.execute('select * from users where id=?',(uid,)).fetchone()
        if r:
            # IMPORTANT: never use shared-port firewall counters for a user reset.
            # All users share one public port, so port counters belong to everyone and
            # would make unused/new proxies show fake usage. Only a real per-secret
            # MTProxy stats counter may be used here.
            base=secret_total(r['secret'])
            con.execute('update users set used_reset_bytes=? where id=?',(base,uid))

def used_bytes(r):
    # Accurate shared-port accounting rule:
    # - If MTProxy exposes a per-secret counter, use it.
    # - If it does not, show 0 instead of guessing from the shared port.
    # Guessing from INPUT/OUTPUT/iptables/nft on the shared port is what caused
    # fresh unused proxies to consume quota.
    total=secret_total(r['secret'])
    return max(0, total-int(r['used_reset_bytes'] or 0))

def traffic_counter_status():
    text=mtproxy_stats_text()
    if not text.strip():
        return 'MTProxy /stats در دسترس نیست؛ برای جلوگیری از مصرف فیک، آمار پورت مشترک استفاده نمی‌شود.'
    return 'MTProxy /stats در دسترس است؛ فقط آمار واقعی Secretها محاسبه می‌شود.'
