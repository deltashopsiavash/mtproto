...
def reset_user_usage(uid):
    with db() as con:
        ...
        if r: con.execute('update users set used_reset_bytes=? where id=?',(traffic_total(row_local_port(r)),uid))
    ...
    if request.method=='POST':
        try:
            ...
            with db() as con:
                ...
                uid=cur.lastrowid; lp=user_local_port(uid); sec=normalize_secret(manual if request.form.get('secret_mode')=='manual' else '', username, uid); sni=secret_sni(sec) or make_sni(username,uid)
                con.execute('update users set secret=?,local_port=?,sni_host=?,used_reset_bytes=? where id=?',(sec,lp,sni,traffic_total(lp),uid))
