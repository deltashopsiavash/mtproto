#!/usr/bin/env python3
import io, json, os, secrets, sqlite3, subprocess, time, threading, re
...
def local_base_port(): return int(cfg().get('local_base_port',20000))
def user_local_port(uid):
    # safe localhost backend port; external clients still use shared_proxy_port()
    return local_base_port()+int(uid)
...
def _safe_name(x):
    x=re.sub(r'[^a-zA-Z0-9-]+','-',str(x or 'user')).strip('-').lower()
    return x[:32] or 'user'

def make_sni(username='', uid=None):
    # This fake TLS SNI is only used internally by HAProxy to route the shared public port
    # to the correct local MTProxy backend. It does not need to resolve in DNS.
    suffix = str(uid) if uid else secrets.token_hex(3)
    return f"{_safe_name(username)}-{suffix}.mtp.local"

def tls_secret(base_secret=None, sni_host=None):
    base=(base_secret or secrets.token_hex(16)).strip().lower()
    if len(base)!=32 or not all(c in '0123456789abcdef' for c in base):
        base=secrets.token_hex(16)
    host=(sni_host or make_sni()).strip().lower().encode().hex()
    return 'ee' + base + host

def secret_sni(secret):
    secret=(secret or '').strip().lower()
    # ee + 16-byte hex secret + fake-domain hex. Telegram sends that fake-domain as TLS SNI.
    if secret.startswith('ee') and len(secret)>34:
        hx=secret[34:]
        try:
            host=bytes.fromhex(hx).decode('ascii','ignore').strip().lower()
...
def normalize_secret(secret, username='', uid=None):
    secret=(secret or '').strip().lower()
    if not secret:
        return tls_secret(sni_host=make_sni(username, uid))
    # If admin entered a classic 32-hex secret, wrap it in TLS-mode with a unique fake SNI.
    # This keeps one public port but lets HAProxy route and count this user separately.
    if len(secret)==32 and all(c in '0123456789abcdef' for c in secret):
        return tls_secret(secret, make_sni(username, uid))
    # Full TLS secret is accepted as-is if it contains a decodable SNI.
    if secret_sni(secret):
        return secret
    raise ValueError('Secret نامعتبر است. برای پورت مشترک، Secret باید خودکار ساخته شود یا ee TLS-secret معتبر باشد.')
...
def row_local_port(r):
    try: return int(r['local_port'] or 0) or user_local_port(r['id'])
    except Exception: return shared_proxy_port()

def reset_user_usage(uid):
    # Avoid nested SQLite write locks: read row, close connection, then write the new baseline.
    with db() as con:
        r=con.execute('select * from users where id=?',(uid,)).fetchone()
    if r:
        baseline=traffic_total(row_local_port(r))
        with db() as con:
            con.execute('update users set used_reset_bytes=? where id=?',(baseline,uid))

def used_bytes(r): return max(0, traffic_total(row_local_port(r))-int(r['used_reset_bytes'] or 0))
...
def init_db():
    ...
    with db() as con:
        ...
        add_col(con,'users','sub_token','text'); add_col(con,'users','used_reset_bytes','integer not null default 0'); add_col(con,'users','quota_bytes','integer not null default 0'); add_col(con,'users','local_port','integer not null default 0'); add_col(con,'users','sni_host','text')
        con.execute('create table if not exists traffic_state(port integer primary key,last_raw integer not null default 0,total_bytes integer not null default 0,updated_at text)')
        ...
        for r in con.execute('select id,username,secret,local_port,sni_host,used_reset_bytes from users').fetchall():
            old_local=int(r['local_port'] or 0)
            old_tls=bool(secret_sni(r['secret']))
            lp=old_local or user_local_port(r['id'])
            sni=r['sni_host'] or secret_sni(r['secret']) or make_sni(r['username'],r['id'])
            sec=r['secret'] if old_tls else normalize_secret(r['secret'], r['username'], r['id'])
            # When migrating from the old shared-port global counter, reset this user's baseline.
            # Otherwise old global bytes would be subtracted from the new per-user local-port counter.
            reset=int(r['used_reset_bytes'] or 0) if (old_local and old_tls) else 0
            con.execute('update users set local_port=?,sni_host=?,secret=?,used_reset_bytes=? where id=?',(lp,sni,sec,reset,r['id']))
...
def restart_shared_proxy():
    """Run one public HAProxy listener and one localhost MTProxy backend per user.
    This preserves a single public port while making traffic counters independent per Secret.
    Routing is based on TLS-mode MTProxy secret SNI (ee + base-secret + fake-domain).
    """
    port=shared_proxy_port(); MTPROXY_DIR.mkdir(parents=True,exist_ok=True)
    cleanup_old_proxy_services()
    sh('systemctl stop mtproxy-shared.service >/dev/null 2>&1 || true')
    # Avoid old-style percent formatting here; it could raise `not enough arguments for format string`
    # if the command pattern is edited later and contains extra percent tokens.
    sh(f"pkill -f '/usr/local/bin/mtproto-proxy.*-H {int(port)}' >/dev/null 2>&1 || true")
    with db() as con:
        rows=con.execute('select * from users where enabled=1 order by id').fetchall()
        # make sure every enabled user has TLS SNI + local backend port
        fixed=[]
        for r in rows:
            lp=int(r['local_port'] or 0) or user_local_port(r['id'])
            sni=(r['sni_host'] or secret_sni(r['secret']) or make_sni(r['username'], r['id'])).strip().lower()
            sec=r['secret'] if secret_sni(r['secret']) else normalize_secret(r['secret'], r['username'], r['id'])
            if secret_sni(sec): sni=secret_sni(sec)
            con.execute('update users set local_port=?,sni_host=?,secret=? where id=?',(lp,sni,sec,r['id']))
            fixed.append(dict(r)|{'local_port':lp,'sni_host':sni,'secret':sec})
        rows=fixed
    # Stop old per-user units that no longer exist.
    active_ids=' '.join(str(r['id']) for r in rows)
    active_ids_q = q(' ' + active_ids + ' ')
    sh(f"for u in $(systemctl list-units 'mtproxy-backend-*' --all --no-legend 2>/dev/null | awk '{{print $1}}'); do id=${{u#mtproxy-backend-}}; id=${{id%.service}}; echo {active_ids_q} | grep -q \" $id \" || systemctl disable --now $u >/dev/null 2>&1 || true; done")
    if not rows:
        sh('systemctl disable --now mtproxy-haproxy.service >/dev/null 2>&1 || true')
        return
    for r in rows:
        lp=int(r['local_port']); ensure_traffic_rule(lp)
        unit=f"""[Unit]\nDescription=MTProxy backend for {r['username']}\nAfter=network.target\n\n[Service]\nType=simple\nExecStart=/usr/local/bin/mtproto-proxy -u nobody -p {int(cfg().get('stats_port',9000))+int(r['id'])} -H {lp} -S {r['secret']} --aes-pwd {MTPROXY_DIR}/proxy-secret {MTPROXY_DIR}/proxy-multi.conf -M 1\nRestart=always\nRestartSec=3\nLimitNOFILE=65535\nKillMode=control-group\nTimeoutStopSec=5\n\n[Install]\nWantedBy=multi-user.target\n"""
        Path(f'/etc/systemd/system/mtproxy-backend-{int(r["id"])}.service').write_text(unit)
    cfg_lines=[
        'global',
        '    log /dev/log local0',
        '    maxconn 200000',
        ...
        '    timeout client  1h',
        '    timeout server  1h',
        'frontend mtproto_shared',
        f'    bind *:{port}',
        '    tcp-request inspect-delay 5s',
        '    tcp-request content accept if { req_ssl_hello_type 1 }',
    ]
    for r in rows:
        cfg_lines.append(f'    use_backend be_{int(r["id"])} if {{ req.ssl_sni -i {r["sni_host"]} }}')
    cfg_lines.append(f'    default_backend be_{int(rows[0]["id"])}')
    for r in rows:
        cfg_lines += [f'backend be_{int(r["id"])}', f'    server s1 127.0.0.1:{int(r["local_port"])} check']
    haproxy_cfg='\n'.join(cfg_lines)+'\n'
    Path('/etc/haproxy').mkdir(parents=True,exist_ok=True)
    Path('/etc/haproxy/mtproto-panel.cfg').write_text(haproxy_cfg)
    Path('/etc/systemd/system/mtproxy-haproxy.service').write_text(f"""[Unit]\nDescription=MTProto Panel shared-port HAProxy router\nAfter=network.target\n\n[Service]\nType=simple\nExecStart=/usr/sbin/haproxy -f /etc/haproxy/mtproto-panel.cfg -db\nRestart=always\nRestartSec=3\nLimitNOFILE=200000\n\n[Install]\nWantedBy=multi-user.target\n""")
    sh('systemctl daemon-reload')
    for r in rows:
        sh(f'systemctl enable --now mtproxy-backend-{int(r["id"])}.service >/dev/null 2>&1 || true')
        sh(f'systemctl restart mtproxy-backend-{int(r["id"])}.service >/dev/null 2>&1 || true')
    sh('systemctl enable --now mtproxy-haproxy.service >/dev/null 2>&1 || true')
    sh('systemctl restart mtproxy-haproxy.service >/dev/null 2>&1 || true')
...
def new_user():
    ...
    if request.method=='POST':
        try:
            username=request.form['username'].strip(); qb,qgb,unit=parse_quota_form(request.form); days=int(request.form.get('expiry_days') or 30); expires=(date.today()+timedelta(days=days)).isoformat(); manual=(request.form.get('secret') or '').strip()
            # Do not call traffic_total() while a users write transaction is open; SQLite can lock.
            with db() as con:
                cur=con.execute('insert into users(username,port,secret,quota_gb,quota_bytes,expires_at,enabled,created_at,note,sub_token,used_reset_bytes,local_port,sni_host) values(?,?,?,?,?,?,?,?,?,?,?,?,?)',(username,port,'pending',qgb,qb,expires,1,datetime.utcnow().isoformat(),request.form.get('note',''),secrets.token_urlsafe(16),0,0,''))
                uid=cur.lastrowid
            lp=user_local_port(uid); sec=normalize_secret(manual if request.form.get('secret_mode')=='manual' else '', username, uid); sni=secret_sni(sec) or make_sni(username,uid)
            baseline=traffic_total(lp)
            with db() as con:
                con.execute('update users set secret=?,local_port=?,sni_host=?,used_reset_bytes=? where id=?',(sec,lp,sni,baseline,uid))
...
def edit_user(uid):
    ...
    if request.method=='POST':
        qb,qgb,unit=parse_quota_form(request.form); days=int(request.form.get('expiry_days') or 30); expires=(date.today()+timedelta(days=days)).isoformat(); enabled=1 if request.form.get('enabled')=='on' else 0; reset=traffic_total(row_local_port(r)) if request.form.get('reset_usage')=='on' else r['used_reset_bytes']; secret=normalize_secret(request.form.get('secret') or r['secret'], r['username'], uid)
        with db() as con: con.execute('update users set port=?,secret=?,sni_host=?,local_port=?,quota_gb=?,quota_bytes=?,expires_at=?,note=?,enabled=?,used_reset_bytes=? where id=?',(shared_proxy_port(),secret,secret_sni(secret) or r['sni_host'] or make_sni(r['username'],uid),row_local_port(r),qgb,qb,expires,request.form.get('note',''),enabled,reset,uid))
...
def import_backup():
    ...
    with db() as con:
        ...
        for u in data.get('users',[]):
            ...
            sec=u.get('secret') or ''
            if not secret_sni(sec): sec=normalize_secret(sec, u['username'])
            sni=secret_sni(sec) or make_sni(u['username'])
            cur=con.execute('insert into users(username,port,secret,quota_gb,quota_bytes,expires_at,enabled,created_at,note,sub_token,used_reset_bytes,local_port,sni_host) values(?,?,?,?,?,?,?,?,?,?,?,?,?)',(u['username'],shared_proxy_port(),sec,qgb,qb,u.get('expires_at') or date.today().isoformat(),int(u.get('enabled',1)),u.get('created_at') or datetime.utcnow().isoformat(),u.get('note',''),u.get('sub_token') or secrets.token_urlsafe(16),0,0,sni))
            uid=cur.lastrowid; lp=int(u.get('local_port') or 0) or user_local_port(uid)
            con.execute('update users set local_port=?,used_reset_bytes=? where id=?',(lp,traffic_total(lp),uid))
