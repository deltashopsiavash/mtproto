...
def reset_user_usage(uid):
    with db() as con:
        con.execute('update users set used_reset_bytes=? where id=?',(traffic_total(shared_proxy_port()),uid))

def used_bytes(r): return max(0, traffic_total(shared_proxy_port())-int(r['used_reset_bytes'] or 0))
...
def restart_shared_proxy():
    ...
    if not args:
        ...
        return
    unit=f"""[Unit]\nDescription=Shared MTProto Proxy Multi Secret\nAfter=network.target\n\n[Service]\nType=simple\nExecStart=/usr/local/bin/mtproto-proxy -u nobody -p {int(cfg().get('stats_port',9000))} -H {port} {args} --aes-pwd {MTPROXY_DIR}/proxy-secret {MTPROXY_DIR}/proxy-multi.conf -M 1\nRestart=always\nRestartSec=3\nLimitNOFILE=65535\nKillMode=control-group\nTimeoutStopSec=5\n\n[Install]\nWantedBy=multi-user.target\n"""
...
def new_user():
    ...
    if request.method=='POST':
        try:
            ...
            with db() as con:
                con.execute('insert into users(username,port,secret,quota_gb,quota_bytes,expires_at,enabled,created_at,note,sub_token,used_reset_bytes) values(?,?,?,?,?,?,?,?,?,?,?)',(username,port,secret,qgb,qb,expires,1,datetime.utcnow().isoformat(),request.form.get('note',''),secrets.token_urlsafe(16),traffic_total(port)))
...
def edit_user(uid):
    ...
    if request.method=='POST':
        qb,qgb,unit=parse_quota_form(request.form); days=int(request.form.get('expiry_days') or 30); expires=(date.today()+timedelta(days=days)).isoformat(); enabled=1 if request.form.get('enabled')=='on' else 0; reset=traffic_total(shared_proxy_port()) if request.form.get('reset_usage')=='on' else r['used_reset_bytes']; secret=normalize_secret(request.form.get('secret') or r['secret'])
...
def import_backup():
    ...
    with db() as con:
        ...
        for u in data.get('users',[]):
            ...
            con.execute('insert into users(username,port,secret,quota_gb,quota_bytes,expires_at,enabled,created_at,note,sub_token,used_reset_bytes) values(?,?,?,?,?,?,?,?,?,?,?)',(u['username'],shared_proxy_port(),u['secret'],qgb,qb,u.get('expires_at') or date.today().isoformat(),int(u.get('enabled',1)),u.get('created_at') or datetime.utcnow().isoformat(),u.get('note',''),u.get('sub_token') or secrets.token_urlsafe(16),int(u.get('used_reset_bytes') or traffic_total(shared_proxy_port()))))
