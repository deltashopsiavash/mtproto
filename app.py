...
def traffic_counter_status():
    text=mtproxy_stats_text()
    if not text.strip():
        return 'آمار واقعی هر Secret از MTProxy دریافت نشد. چون همه کاربران روی یک پورت مشترک هستند، پنل عمداً آمار کل پورت را بین کاربران تقسیم نمی‌کند تا مصرف فیک ساخته نشود. برای مصرف دقیق هر کاربر، موتور پروکسی باید user-aware باشد یا هر کاربر پورت جدا داشته باشد.'
    # Check whether /stats contains any active user Secret. Official MTProxy usually exposes only global counters.
    try:
        with db() as con:
            rows=con.execute('select secret from users where enabled=1').fetchall()
        low=text.lower()
        if any(any(v and v in low for v in _secret_variants(r['secret'])) for r in rows):
            return 'MTProxy /stats در دسترس است و Secretها در خروجی دیده می‌شوند؛ مصرف بر اساس Secret محاسبه می‌شود.'
    except Exception:
        pass
    return 'MTProxy /stats در دسترس است، اما خروجی آن Secret کاربران را جدا نشان نمی‌دهد. در حالت پورت مشترک، آمار کل پورت قابل تفکیک نیست؛ برای جلوگیری از مصرف اشتباه، مصرف کاربران ۰ می‌ماند تا آمار فیک ثبت نشود.'
...
@app.route('/traffic-debug')
@login_required
def traffic_debug():
    text=mtproxy_stats_text()
    body='<h2>Traffic Debug</h2><div class="card"><p class="mini-help">این صفحه برای عیب‌یابی منبع آمار مصرف است. اگر در Raw /stats هیچ Secret کاربری دیده نشود، MTProxy رسمی امکان تفکیک حجم کاربران روی یک پورت مشترک را نمی‌دهد.</p>'
    body += '<p><b>وضعیت:</b> '+traffic_counter_status()+'</p>'
    safe=(text or '(empty)').replace('&','&amp;').replace('<','&lt;').replace('>','&gt;')
    body += '<h3>Raw /stats</h3><pre style="direction:ltr;text-align:left;white-space:pre-wrap;background:var(--input);padding:12px;border-radius:12px">'+safe+'</pre></div>'
    return page(body)
