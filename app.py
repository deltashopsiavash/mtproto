...
def _numbers_from_line(line):
    vals=[]
    for x in re.findall(r'(?<![A-Za-z0-9])([0-9]{2,})(?![A-Za-z0-9])', line):
...
def _line_looks_like_traffic(line):
    l=line.lower()
    traffic_words=('byte','bytes','traffic','transferred','upload','download','in_bytes','out_bytes','sent','recv','received')
...
def secret_total_raw(secret):
    ...
    Different MTProxy builds expose tab-separated stats with slightly different
    key names. This parser intentionally accepts any line that contains the secret
    and a byte/traffic metric. It never uses the shared-port firewall counter,
    because that cannot be divided between secrets.
    """
    ...
    for line in text.splitlines():
        ...
        if not any(v and v in low for v in variants):
            continue
        if not _line_looks_like_traffic(line):
            continue
        nums=_numbers_from_line(line)
        if nums:
            # Usually a per-secret stats line has one byte value; if it has in/out
            # bytes on the same line, summing is what we want for quota usage.
            total += sum(nums)
    return max(0,total)
