...
def reset_user_usage(uid):
    with db() as con:
        ...
        if r:
            base=secret_total(r['secret']) or _allocated_user_total(r['username'])
            con.execute('update users set used_reset_bytes=? where id=?',(base,uid))

def used_bytes(r):
    # Prefer real per-secret /stats when available. If the official MTProxy build
    # does not expose it, use the safe shared-port allocator so usage does not stay at 0.
    real=secret_total(r['secret'])
    total=real if real>0 else _allocated_user_total(r['username'])
