...
def restart_shared_proxy():
    ...
    sh("for u in $(systemctl list-units 'mtproxy-backend-*' --all --no-legend 2>/dev/null | awk '{print $1}'); do id=${u#mtproxy-backend-}; id=${id%.service}; echo ' %s ' | grep -q \" $id \" || systemctl disable --now $u >/dev/null 2>&1 || true; done" % active_ids)
    if not rows:
        sh('systemctl disable --now mtproxy-haproxy.service >/dev/null 2>&1 || true')
        return
    for r in rows:
        lp=int(r['local_port']); ensure_traffic_rule(lp)
        unit=f"""[Unit]\nDescription=MTProxy backend for {r['username']}\nAfter=network.target\n\n[Service]\nType=simple\nExecStart=/usr/local/bin/mtproto-proxy -u nobody -p {int(cfg().get('stats_port',9000))+int(r['id'])} -H {lp} -S {q(r['secret'])} --aes-pwd {MTPROXY_DIR}/proxy-secret {MTPROXY_DIR}/proxy-multi.conf -M 1\nRestart=always\nRestartSec=3\nLimitNOFILE=65535\nKillMode=control-group\nTimeoutStopSec=5\n\n[Install]\nWantedBy=multi-user.target\n"""
