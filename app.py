...
def used_bytes_raw(port):
    # Raw firewall counters for the shared MTProto port only. This can reset after firewall/service reload,
    # so never show it directly to users; traffic_total() converts it to a stable increasing counter.
    ...
    for chain,lab in (('INPUT','in'),('OUTPUT','out')):
        ...
        try: total += max(0,int(float(out.splitlines()[-1] if out.splitlines() else 0)))
        except Exception: pass
    return total
