...
def create_user(username, amount, unit, days, secret_mode='auto', secret_value=''):
    ...
    secret = panel.normalize_secret('' if secret_mode=='auto' else secret_value)
    exp=(date.today()+timedelta(days=int(days))).isoformat()
    now=datetime.utcnow().isoformat(); token=secrets.token_urlsafe(16); baseline=panel.secret_total(secret)
    with panel.db() as con:
        con.execute('insert into users(username,port,secret,quota_gb,quota_bytes,expires_at,enabled,created_at,sub_token,used_reset_bytes) values(?,?,?,?,?,?,?,?,?,?)',
                    (username, panel.shared_proxy_port(), secret, qgb, qb, exp, 1, now, token, baseline))
...
def handle_text(uid, chat_id, text):
    ...
    try:
        ...
        if step=='new_username':
            ...
            sec=panel.normalize_secret(text.strip())
            with panel.db() as con: con.execute('update users set secret=?,used_reset_bytes=? where id=?',(sec,panel.secret_total(sec),st['uid']))
