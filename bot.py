...
def create_user(username, amount, unit, days, secret_mode='auto', secret_value=''):
    ...
    with panel.db() as con:
        cur=con.execute('insert into users(username,port,secret,quota_gb,quota_bytes,expires_at,enabled,created_at,sub_token,used_reset_bytes,local_port,sni_host) values(?,?,?,?,?,?,?,?,?,?,?,?,?)',
                    (username, panel.shared_proxy_port(), 'pending', qgb, qb, exp, 1, now, token, 0, 0, ''))
        uid=cur.lastrowid; lp=panel.user_local_port(uid); secret=panel.normalize_secret('' if secret_mode=='auto' else secret_value, username, uid); sni=panel.secret_sni(secret) or panel.make_sni(username, uid)
        con.execute('update users set secret=?,local_port=?,sni_host=?,used_reset_bytes=? where id=?',(secret,lp,sni,panel.traffic_total(lp),uid))
